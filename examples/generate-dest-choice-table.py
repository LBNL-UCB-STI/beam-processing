...
tours = tours[
    ["person_id", "tour_type", "origin", "destination", "household_id", "tour_mode"]
].copy()
tours.tour_mode = tours.tour_mode.replace(
    {
        "SHARED3FREE": "DRIVE",
        "SHARED3PAY": "DRIVE",
        "SHARED2FREE": "DRIVE",
        "SHARED2PAY": "DRIVE",
        "DRIVEALONEFREE": "DRIVE",
        "DRIVEALONEPAY": "DRIVE",
    }
)
tours = tours.loc[
    tours.tour_mode.isin(
        ["DRIVE", "WALK", "WALK_LOC", "WALK_HVY", "WALK_COM", "WALK_LRF"]
    ),
    :,
]
t = tours.head(1000)
...
chosen_outcomes = skims.reindex(
    pd.MultiIndex.from_frame(t[["origin", "destination"]].astype(int))
)
