...
FUEL_CONVERSION_JOULES_GALLON_DIESEL = 8.3141841e-9  # MJ/gallon * J/MJ # Same note as above.
...
class OutputDataFrame:
    ...
    def _write(self, obj):
        ...
        try:
            ...
            print(f"Error writing {self.__class__.__name__} to {self._diskLocation}: {e}")
    ...
    def _read(self):
        ...
        if not os.path.exists(self._diskLocation):
             ...
             print(f"Cache file does not exist at {self._diskLocation} during read attempt.")
             return None # Indicate read failure

        try:
            return pd.read_parquet(self._diskLocation, engine="fastparquet")
        except Exception as e:
            print(f"Error reading {self.__class__.__name__} from {self._diskLocation}: {e}")
    ...
    def dataFrame(self) -> Optional[pd.DataFrame]:
        ...
        if self._dataFrame is None:
            ...
            if self._dataFrame is None:
                print(f"Cache miss or failed read for {self.__class__.__name__}. Loading raw data.")
                df = self.load()
                if df is None:
                    print(f"Loading raw data for {self.__class__.__name__} returned None.")
                    return None # Loading failed

                print(f"Preprocessing raw data for {self.__class__.__name__} ({df.shape[0]} rows)...")
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"No custom preprocessing defined for {self.__class__.__name__}. Returning raw data.")
        return df
    ...
    def toCsv(self):
        ...
        if df_to_save is not None and not df_to_save.empty:
             ...
             print(f"No dataFrame available or dataFrame is empty for {name}. Not saving CSV.")
        ...
        if df is not None:
             try:
                 ...
                 if fromCol in df.index.names:
                     df[toCol] = df.index.get_level_values(fromCol).map(
                         mapping
                     )
                 else:
                     print(f"Warning: Column '{fromCol}' not found in DataFrame index levels.")
                     # Optionally try mapping from a column if not in index
                     if fromCol in df.columns:
                          df[toCol] = df[fromCol].map(mapping)
                     else:
                          print(f"Warning: Column '{fromCol}' not found in DataFrame columns either. Cannot add mapping.")
        ...
        if df is not None:
             try:
                ...
                if col in df.columns:
                 ...
                 print(f"Error unstacking column '{col}' by index '{index}' for {self.__class__.__name__}: {e}")
...
class PathTraversalEvents(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        Returns:
            pd.DataFrame: The preprocessed DataFrame.
        """
        print(f"Preprocessing PathTraversalEvents using fixPathTraversals ({df.shape[0]} rows)...")
    ...
    def _write(self, obj: Dict[str, pd.DataFrame]):
        ...
        for grp, df in obj.items():
            if not isinstance(df, pd.DataFrame):
                 print(f"Warning: Item '{grp}' in dictionary is not a DataFrame. Skipping write for this item.")
                 continue
    ...
    def _read(self) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        for tab in self.__requiredTables:
            ...
            try:
                ...
                print(f"Error reading required cache file {file_loc} for table '{tab}': {e}")
                read_success = False
        ...
        if self._dataFrame is None:
            ...
            if self.cached:
                ...
                if self._dataFrame is not None:
                     print(f"Successfully read {self.__class__.__name__} from cache.")
            if self._dataFrame is None:
                print(f"Cache miss or failed read for {self.__class__.__name__}. Loading raw events.")
                ...
                for tab in self.__requiredTables:
                    if tab in self.beamInputDirectory.eventsFile.eventTypes:
                        raw_events_dict[tab] = self.beamInputDirectory.eventsFile.eventTypes[tab]
                        print(f"Loaded {raw_events_dict[tab].shape[0]} raw events for {tab}.")
                    else:
                        print(f"Warning: Raw event type '{tab}' not found after collection attempt.")
                ...
                if not all(tab in raw_events_dict and not raw_events_dict[tab].empty for tab in essential_events):
                    print(f"Error: Essential raw event types ({essential_events}) not loaded or are empty. Cannot preprocess.")
...
class RealizedModeCount(OutputDataFrame):
    def __init__(
        ...
        **kwargs,
        ...
        # Pass args/kwargs up to OutputDataFrame
        super().__init__(outputDataDirectory, modeChoiceEvents.inputDirectory, *args, **kwargs)
        self.modeChoiceEvents = modeChoiceEvents
        self.beamInputDirectory = modeChoiceEvents.inputDirectory # Redundant, inputDirectory is already set by super
        self.indexedOn = "mode"

    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        print(f"Preprocessing RealizedModeCount from ModeChoice events ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            print("Input DataFrame for RealizedModeCount is empty or None.")
            return pd.DataFrame(columns=["RealizedTrips"], index=pd.Index([], name="mode"))
        ...
        try:
            ...
            if 'tripId' in df.columns:
                 ...
                 if 'mode' in df_filtered.columns:
                    ...
                    print(f"Finished preprocessing RealizedModeCount ({result_df.shape[0]} rows).")
                    return result_df
                 else:
                    print("Warning: 'mode' column not found in ModeChoiceEvents DataFrame.")
                    return pd.DataFrame(columns=["RealizedTrips"], index=pd.Index([], name="mode"))

            else:
                 print("Warning: 'tripId' column not found in ModeChoiceEvents DataFrame. Cannot count realized modes per trip.")
                 return pd.DataFrame(columns=["RealizedTrips"], index=pd.Index([], name="mode"))

        except Exception as e:
            print(f"Error during RealizedModeCount preprocessing: {e}")
            return pd.DataFrame(columns=["RealizedTrips"], index=pd.Index([], name="mode")) # Return empty on error
...
class ModeVMT(OutputDataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # Pass args/kwargs up to OutputDataFrame
        super().__init__(outputDataDirectory, pathTraversalEvents.inputDirectory, *args, **kwargs)
    ...
    def preprocess(self, df):
        ...
        if df is None or df.empty:
            ...
            return pd.DataFrame(columns=["vehicleMiles"], index=pd.Index([], name="mode_extended"))

        # Ensure required columns exist
        if 'mode_extended' not in df.columns or 'vehicleMiles' not in df.columns:
            print("Error: Required columns 'mode_extended' or 'vehicleMiles' not found for ModeVMT aggregation.")
            return pd.DataFrame(columns=["vehicleMiles"], index=pd.Index([], name="mode_extended"))
...
class ModeVHT(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        if df is None or df.empty:
            ...
            return pd.DataFrame(columns=["vehicleHours"], index=pd.Index([], name="mode_extended"))
        ...
        if 'mode_extended' not in df.columns:
             print("Error: Required column 'mode_extended' not found for ModeVHT aggregation.")
             return pd.DataFrame(columns=["vehicleHours"], index=pd.Index([], name="mode_extended"))
        ...
        try:
            ...
            df_agg = df_valid_duration.groupby("mode_extended").agg(
                vehicleHours=("duration", lambda x: x.sum() / 3600.0) # Assuming duration is in seconds
            )
            df_agg.index.name = self.indexedOn
...
class PassengerMilesByVehicleAndMode(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        # Add a print statement for clarity.
        print(f"PassengerMilesByVehicleAndMode preprocess step (no-op). Input shape: {df.shape}")
        return df
    ...
    def load(self):
        ...
        PTs = self.pathTraversalEvents.dataFrame
        # Clear memory of the source object after accessing its data
        # Consider if this is truly necessary/safe here given caching
        # self.pathTraversalEvents.clearMemory()

        # Ensure df is not None or empty before processing
        if PTs is None or PTs.empty:
            print("Input DataFrame for PassengerMilesByVehicleAndMode is empty or None.")
        ...
        # Ensure required columns exist
        required_cols = ['vehicleType', 'mode_extended', 'passengerMiles']
        if not all(col in PTs.columns for col in required_cols):
             print(f"Error: Required columns {required_cols} not found for PassengerMilesByVehicleAndMode aggregation.")
             return pd.DataFrame(
                columns=[],
                index=pd.Index([], name="vehicleType")
            )
        ...
        try:
            # Fill NaN in relevant columns if necessary, although fixPathTraversals should handle mode_extended.
            # vehicleType should also be present from BEAM events.
            # PTs.fillna({'vehicleType': 'Unknown', 'mode_extended': 'Unknown'}, inplace=True) # Example

            # Group by vehicleType and mode_extended and sum passengerMiles
            modeDistances = PTs.groupby(["vehicleType", "mode_extended"]).agg(
                {"passengerMiles": "sum"}
            )
            ...
            if not modeDistances.empty:
                 ...
                 modeDistances.index.name = "vehicleType"  # The remaining index after unstacking
            else:
                 print("Warning: Groupby result for PassengerMilesByVehicleAndMode is empty.")
                 # Return empty DataFrame with expected index name
                 modeDistances = pd.DataFrame(index=pd.Index([], name="vehicleType"))

            print(f"Finished aggregating PassengerMilesByVehicleAndMode ({modeDistances.shape[0]} rows).")
...
class ReplanningEventReasons(OutputDataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.indexedOn = None # Index is not set in this class, often just a list of reasons
    ...
    def load(self):
        ...
        Returns:
            pd.DataFrame: The loaded DataFrame.
        """
        print(f"Loading ReplanningEventReasons from {self.inputDirectory.replanningEventReasonFile.filePath}...")
    ...
    def preprocess(self, df):
        ...
        """
        print(f"ReplanningEventReasons preprocess step (no-op). Input shape: {df.shape if df is not None else 'None'}")
        return df
...
class ScoreStats(OutputDataFrame):
    ...
    def load(self):
        ...
        Returns:
            pd.DataFrame: The loaded DataFrame.
        """
        print(f"Loading ScoreStats from {self.inputDirectory.scoreStatsFile.filePath}...")
        # Accessing .file() on RawOutputFile triggers its load/cache logic
    ...
    def preprocess(self, df):
         ...
         """
         print(f"ScoreStats preprocess step (no-op). Input shape: {df.shape if df is not None else 'None'}")
         return df
...
class ModeEnergy(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        Returns:
            pd.DataFrame: The aggregated DataFrame.
        """
        print(f"Aggregating ModeEnergy from PathTraversalEvents ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            print("Input DataFrame for ModeEnergy is empty or None.")
            return pd.DataFrame(columns=["totalEnergyInJoules"], index=pd.Index([], name="mode_extended"))

        # Ensure required columns exist
        if 'mode_extended' not in df.columns or 'totalEnergyInJoules' not in df.columns:
             print("Error: Required columns 'mode_extended' or 'totalEnergyInJoules' not found for ModeEnergy aggregation.")
             return pd.DataFrame(columns=["totalEnergyInJoules"], index=pd.Index([], name="mode_extended"))

        try:
            # Group by the correct mode_extended column and sum totalEnergyInJoules
            # Ensure totalEnergyInJoules column is numeric
            df['totalEnergyInJoules'] = pd.to_numeric(df['totalEnergyInJoules'], errors='coerce').fillna(0)
...
class ModePMT(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        if df is None or df.empty:
            ...
            return pd.DataFrame(columns=["passengerMiles"], index=pd.Index([], name="mode_extended"))

        # Ensure required columns exist
        if 'mode_extended' not in df.columns or 'passengerMiles' not in df.columns:
             print("Error: Required column 'mode_extended' or 'passengerMiles' not found for ModePMT aggregation.")
             return pd.DataFrame(columns=["passengerMiles"], index=pd.Index([], name="mode_extended"))

        try:
            # Group by the correct mode_extended column and sum passengerMiles
            # Ensure passengerMiles column is numeric
            df['passengerMiles'] = pd.to_numeric(df['passengerMiles'], errors='coerce').fillna(0)
...
class LinkStatsFromPathTraversals(OutputDataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            iteration (int): The BEAM iteration number.
        """
        # Include iteration in hash key
        # Pass args/kwargs up to OutputDataFrame
        super().__init__(outputDataDirectory, pathTraversalEvents.inputDirectory, *args, **kwargs)

        self.indexedOn = ["link", "hour"] # This is the index *after* preprocessing
        self.pathTraversalEvents = pathTraversalEvents
        self.iteration = iteration # Store iteration

        # Override disk location hash *after* super() initializes it,
        # but using a hash that includes iteration.
        # Alternatively, calculate hash *before* super and pass as hash_key.
        # Let's try passing it as hash_key to OutputDataFrame.__init__
        # Recalculate hash using iteration and pass to super.
        m = hashlib.md5()
        m.update(self.inputDirectory.directoryPath.encode())
        m.update(self.__class__.__name__.encode())
        m.update(str(self.iteration).encode())
        override_hash = m.hexdigest()

        # Re-initialize using super with the hash_key - this is complex to do cleanly.
        # A simpler way in MI is to let the base class use its default hash, and override
        # the hash() method in the subclass. The _diskLocation will then be calculated
        # based on the overridden hash() when accessed (if not already set).
        # The OutputDataFrame property getter already uses self.hash() implicitly.
        # So just overriding hash() is sufficient if _diskLocation is calculated lazily.
        # OutputDataFrame.__init__ calculates _diskLocation based on hash() immediately.
        # So we *must* override hash() *before* calling super(), or pass the hash key.
        # Passing hash_key seems cleaner. Let's adjust OutputDataFrame to accept hash_key.
        # This change is already done in the OutputDataFrame class definition above.
        # So, let's pass the calculated hash to the super call.

    # Redefine init to pass the iteration-specific hash key
    def __init__(
        self,
        outputDataDirectory: "OutputDataDirectory",
        pathTraversalEvents: PathTraversalEvents,
        ...
        **kwargs,
        ...
        self.iteration = iteration # Must store iteration before hashing

        # Calculate the iteration-specific hash key
        m = hashlib.md5()
        # Use the source's input directory for the path part of the hash
        m.update(pathTraversalEvents.inputDirectory.directoryPath.encode())
        m.update(self.__class__.__name__.encode())
        m.update(str(self.iteration).encode()) # Include iteration in hash
        override_hash = m.hexdigest()

        # Pass the calculated hash_key to the superclass OutputDataFrame
        super().__init__(
            outputDataDirectory,
            pathTraversalEvents.inputDirectory, # Use source's inputDirectory
            hash_key=override_hash, # Pass the custom hash
            *args,
            **kwargs,
        )

        self.indexedOn = ["link", "hour"] # This is the index *after* preprocessing
        self.pathTraversalEvents = pathTraversalEvents # Store source object


    # The hash method in OutputDataFrame is now updated to use _hash_key if provided.
    # So no need to override hash() here explicitly if passing hash_key to super.
    # def hash(self):
    #     """
    #     Generates a hash based on the input directory path, class name, and iteration.
    #     """
    #     # This method is replaced by the hash_key passed in __init__ using the updated OutputDataFrame.hash logic.
    #     pass # Should not be reached if hash_key is set


    def load(self):
        """
        Loads the preprocessed path traversal events data from the source.
        The aggregation logic happens in preprocess().
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: DataFrame of link volumes and travel times per hour.
        """
        print(f"Preprocessing LinkStatsFromPathTraversals using getLinkStats ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            ...
            return pd.DataFrame(
                ...
                index=pd.MultiIndex.from_tuples([], names=['link', 'hour'])
            )
        try:
            result_df = getLinkStats(df)
            print(f"Finished preprocessing LinkStatsFromPathTraversals ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
...
class LabeledNetwork(OutputDataFrame):
    def __init__(
        ...
        **kwargs,
        ...
        self.beamOutputData = beamOutputData # Redundant, inputDirectory is already set by super
    ...
    def preprocess(self, df):
        ...
        """
        print(f"Preprocessing LabeledNetwork using labelNetworkWithTaz ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            print("Input DataFrame for LabeledNetwork is empty or None.")
            return pd.DataFrame(
                columns=['toLocationX', 'toLocationY', 'linkLength', 'linkFreeSpeed', 'linkCapacity', 'numberOfLanes', 'linkModes', 'attributeOrigId', 'attributeOrigType'],
                index=pd.Index([], name="linkId") # Match expected input index
            )
        # Ensure geometry is available
        if self.beamOutputData.geometry is None or self.beamOutputData.geometry.gdf is None:
        ...
        if not all(col in df.columns for col in required_cols):
             print(f"Error: Required columns {required_cols} not found in network DataFrame for labeling.")
             return df
    ...
    def load(self):
        ...
        """
        print(f"Loading raw network file from {self.beamOutputData.networkFile.filePath}...")
        # Accessing .file() on RawOutputFile triggers its load/cache logic
...
class ProcessedPersonsFile(OutputDataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.activitySimOutputData = activitySimOutputData # Redundant, inputDirectory is already set by super
    ...
    def preprocess(self, df):
        ...
        Returns:
            pd.DataFrame: The preprocessed DataFrame.
        """
        print(f"Preprocessing ProcessedPersonsFile using filterPersons ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            ...
            expected_cols = ["earning", "worker", "student", "household_id", "school_zone_id", "age", "work_zone_id", "TAZ", "home_x", "home_y"]
            return pd.DataFrame(columns=expected_cols, index=pd.Index([], name="person_id"))

        try:
            result_df = filterPersons(df)
            print(f"Finished preprocessing ProcessedPersonsFile ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
    ...
    def load(self):
        ...
        Returns:
            pd.DataFrame: The loaded DataFrame.
        """
        print(f"Loading raw persons file from {self.activitySimOutputData.personsFile.filePath}...")
        # Accessing .file() on RawOutputFile triggers its load/cache logic
...
class ProcessedHouseholdsFile(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        Returns:
            pd.DataFrame: The preprocessed DataFrame.
        """
        print(f"Preprocessing ProcessedHouseholdsFile using filterHouseholds ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            ...
            expected_cols = ["recent_mover", "num_workers", "sf_detached", "tenure", "race_of_head", "income", "block_id", "cars", "hhsize", "TAZ", "num_drivers", "num_children"]
            return pd.DataFrame(columns=expected_cols, index=pd.Index([], name="household_id"))

        try:
            result_df = filterHouseholds(df)
            print(f"Finished preprocessing ProcessedHouseholdsFile ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
    ...
    def load(self):
        ...
        Returns:
            pd.DataFrame: The loaded DataFrame.
        """
        print(f"Loading raw households file from {self.activitySimOutputData.householdsFile.filePath}...")
        # Accessing .file() on RawOutputFile triggers its load/cache logic
...
class ProcessedTripsFile(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        Returns:
            pd.DataFrame: The preprocessed DataFrame.
        """
        print(f"Preprocessing ProcessedTripsFile using filterTrips ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            ...
            expected_cols = ["person_id", "household_id", "tour_id", "primary_purpose", "purpose", "destination", "origin", "destination_logsum", "depart", "trip_mode", "mode_choice_logsum"]
            return pd.DataFrame(columns=expected_cols, index=pd.Index([], name="trip_id"))

        try:
            result_df = filterTrips(df)
            print(f"Finished preprocessing ProcessedTripsFile ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
    ...
    def load(self):
        ...
        Returns:
            pd.DataFrame: The loaded DataFrame.
        """
        print(f"Loading raw trips file from {self.activitySimOutputData.tripsFile.filePath}...")
        # Accessing .file() on RawOutputFile triggers its load/cache logic
...
class ProcessedToursFile(OutputDataFrame):
    ...
    def preprocess(self, df):
        ...
        Returns:
            pd.DataFrame: The preprocessed DataFrame.
        """
        print(f"Preprocessing ProcessedToursFile using filterTours ({df.shape[0]} rows if not None)...")
        ...
        if df is not None and df.empty:
             ...
             expected_cols = ["person_id", "tour_type", "tour_category", "number_of_participants", "destination", "origin", "household_id", "start", "end", "duration", "composition", "destination_logsum", "tour_mode", "mode_choice_logsum", "atwork_subtour_frequency", "parent_tour_id", "stop_frequency", "primary_purpose"]
             return pd.DataFrame(columns=expected_cols, index=pd.Index([], name="tour_id"))
        try:
            result_df = filterTours(df) # filterTours returns empty DF for None input
            if result_df is not None:
                print(f"Finished preprocessing ProcessedToursFile ({result_df.shape[0]} rows).")
            else:
    ...
    def load(self):
        ...
        Returns:
            pd.DataFrame: The loaded DataFrame.
        """
        print(f"Loading raw tours file from {self.activitySimOutputData.toursFile.filePath}...")
        # Accessing .file() on RawOutputFile triggers its load/cache logic
...
class ProcessedSkimsFile(OutputDataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pilatesOutputData = pilatesOutputData # Redundant, inputDirectory is already set by super
    ...
    def preprocess(self, df):
        ...
        """
        print(f"ProcessedSkimsFile preprocess step (no-op). Input shape: {df.shape if df is not None else 'None'}")
        return df
...
class TAZBasedDataFrame(OutputDataFrame):
    ...
    def process(
        ...
        if mapping is None and (aggregateBy is not None and len(aggregateBy) > 0):
             raise ValueError("mapping dictionary must be provided if aggregateBy is used.")
        ...
        geom_attrs_in_agg = set(aggregateBy or []).intersection({"county", "areatype10", self.geoIndex})
        if geom_attrs_in_agg:
             geometry_needed = True
             # Add geom attributes to additionalColumns if they aren't already output columns or grouper
             additionalColumns.update(geom_attrs_in_agg - outputColumns - set(aggregateBy or []))

        # If geometry is needed, merge with gdf BEFORE grouping/aggregation
        if geometry_needed:
            if self.geometry is None or self.geometry.gdf is None:
                raise AttributeError("You need to define a geometry (with gdf) to perform area normalization or aggregate by geographic attributes.")
            ...
            geom_cols_present = [col for col in geom_cols if col in self.geometry.gdf.columns]
            ...
            if self.geoIndex not in temp_reset.columns:
                ...
                # A common pattern is for the index level name to become the column name.
                 print(f"Warning: Column '{self.geoIndex}' not found directly after reset_index(). Assuming it was the original index name.")
                 ...
                 if merge_col_left not in temp_reset.columns:
                     print(f"Error: Could not find column '{self.geoIndex}' or equivalent in reset DataFrame for merging.")
            ...
            if merge_col_right not in self.geometry.gdf.columns:
                 print(f"Error: Geometry GDF does not have column '{merge_col_right}' for merging.")
                 return pd.DataFrame() # Return empty on error


            temp_merged = temp_reset.merge(
                self.geometry.gdf[geom_cols_present], # Select only needed columns from gdf
                left_on=merge_col_left,
                right_on=merge_col_right,
                how="left", # Keep all rows from temp_reset
                suffixes=('', '_geom') # Add suffix to gdf columns just in case of name conflicts
            )

            # Drop the duplicate merge key column from the right side if it exists and has a suffix
            if f'{merge_col_right}_geom' in temp_merged.columns and merge_col_left != merge_col_right:
        ...
        if aggregateBy is not None and len(aggregateBy) > 0:
            ...
            if missing_agg_cols:
                 print(f"Error: Aggregation columns {missing_agg_cols} not found in DataFrame after merging.")
                 return pd.DataFrame() # Return empty on error
        ...
        if mapping is not None:
            ...
            missing_mapping_cols = [col for col in mapping_cols if col not in temp.columns]
            if missing_mapping_cols:
                 print(f"Error: Mapping columns {missing_mapping_cols} not found in DataFrame for aggregation.")
                 return pd.DataFrame() # Return empty on error
            ...
            if grouper is not None:
                ...
                # Revisit this case if needed. For now, assume mapping is only used with aggregateBy.
                 print("Warning: Mapping provided without aggregateBy. Applying aggregation to the whole DataFrame.")
                 ...
                 for col, func in mapping.items():
                     if callable(func):
                          ...
                          agg_results[col] = getattr(temp[col], func)() # e.g., temp['colA'].sum()
                     else:
                          print(f"Warning: Unknown aggregation function type for column {col}: {func}")
        ...
        if normalize is not None:
            ...
            if any(func == 'area' for func in normalize.values()) and 'gacres' not in temp.columns:
                 # This should not happen if 'gacres' was added to mapping when needed.
                 # But check defensively.
                 print("Error: 'gacres' column not available for area normalization after aggregation.")
                 return pd.DataFrame() # Return empty on error

            # Ensure columns to normalize exist
            cols_to_normalize = list(normalize.keys())
            missing_norm_cols = [col for col in cols_to_normalize if col not in temp.columns]
            if missing_norm_cols:
                 print(f"Error: Columns to normalize {missing_norm_cols} not found in DataFrame.")
                 return pd.DataFrame() # Return empty on error
            ...
            for col, fn in normalize.items():
                if fn == "area":
                    ...
                    raise NotImplementedError(
                        "Don't have normalization function {0} implemented yet".format(fn)
                    )
        ...
        if 'gacres' in final_cols and 'gacres' not in outputColumns: # If gacres was added for calculation but not desired in output
    ...
    def toGdf(self):
        ...
        if isinstance(df_to_merge.index, pd.MultiIndex):
             ...
             if self.geoIndex not in df_to_merge.index.names:
                  print(f"Error: Geo-index level '{self.geoIndex}' not found in MultiIndex.")
             ...
             print(f"Error: DataFrame index name is not '{self.geoIndex}' and it's not a MultiIndex with that level.")
             print("Attempting to merge on the first column after reset_index() if index was unnamed.")
             df_to_merge = df_to_merge.reset_index()
        ...
        if merge_col_left not in df_to_merge.columns:
             print(f"Error: Merge column '{merge_col_left}' not found in DataFrame after preparing for merge.")
             return gpd.GeoDataFrame() # Cannot proceed
        ...
        # Drop the duplicate merge key column from the right side if it exists and has a suffix
        if f'{merge_col_left}_df' in gdf_merged.columns and merge_col_left != merge_col_right:
...
class EitherLinkStatsFile(OutputDataFrame):
    """
    Represents a LinkStats dataset that can originate either from a raw linkstats file
    or be calculated from PathTraversalEvents. Handles the difference in source.
    Inherits from OutputDataFrame.

    Attributes:
        outputDataDirectory (OutputDataDirectory): The output data directory.
        inputDirectory (InputDirectory): The input directory (from the source).
        source (Union[LinkStatsFromPathTraversals, "LinkStatsFile"]): The source object for the data.
        indexedOn (List[str]): The expected index names for the link stats data ('link', 'hour').
    """
    def __init__(
        self,
        outputDataDirectory: "OutputDataDirectory",
        inputDirectory: InputDirectory, # Explicitly include common args
        ...
        **kwargs, # Accepts args for OutputDataFrame and other parents in MI
        ...
        # Calculate the hash key based on input directory (from source)
        # and the source object type/iteration.
        input_dir_from_source = source.inputDirectory # Get input dir from source object
        source_hash_part = f"{source.__class__.__name__}_{getattr(source, 'iteration', 'no_iter')}"
        m = hashlib.md5()
        input_path_str = str(getattr(input_dir_from_source, 'directoryPath', '')) # Use input_dir_from_source
        m.update(input_path_str.encode())
        m.update(self.__class__.__name__.encode())
        m.update(source_hash_part.encode())
        override_hash = m.hexdigest()
        ...
        self.indexedOn = ["link", "hour"] # This is the expected index for linkstats

    # The hash method in OutputDataFrame is now updated to use _hash_key if provided.
    # No need to override hash() here explicitly if passing hash_key to super.
    # def hash(self):
    #     # ... hash calculation logic (already handled in __init__ and passed as hash_key)
    #     pass # Should not be reached if hash_key is set


    def load(self):
        """
        Loads the DataFrame from the specified source (LinkStatsFromPathTraversals or LinkStatsFile).
        """
        print(f"Loading LinkStats data from source: {self.source.__class__.__name__}...")
        # Accessing .dataFrame or .file() on the source triggers its load/preprocess/cache logic
        if isinstance(self.source, LinkStatsFromPathTraversals):
            df = self.source.dataFrame
        elif isinstance(self.source, LinkStatsFile):
            df = self.source.file()
        else:
             print(f"Error: Unknown source type {self.source.__class__.__name__} for EitherLinkStatsFile.")
             df = None # Indicate loading failure

        if df is not None:
             print(f"Loaded LinkStats data ({df.shape[0]} rows) from source.")
             # Ensure the index is correct after loading
             # LinkStatsFile has index_col=['link', 'hour']
             # LinkStatsFromPathTraversals preprocess() sets index ['link', 'hour']
             # So the index should be correct here.
             if not isinstance(df.index, pd.MultiIndex) or list(df.index.names) != self.indexedOn:
                 print(f"Warning: Loaded LinkStats data does not have expected index {self.indexedOn}. Attempting to reset/set index.")
                 # Try to set index if columns exist
                 if all(col in df.columns for col in self.indexedOn):
                      df = df.reset_index().set_index(self.indexedOn) # Reset index if it wasn't set correctly
                 else:
                      print(f"Error: Cannot set index {self.indexedOn} as required columns are missing.")
                      return None # Indicate loading failure

        else:
             print("Failed to load LinkStats data from source.")

        return df

    # The preprocess method for EitherLinkStatsFile should likely be defined
    # in subclasses that perform specific aggregations (like NetworkVolumesByLink).
    # The default preprocess() from OutputDataFrame (which does nothing) is used here.


class NetworkVolumesByLink(EitherLinkStatsFile):
    """
    Aggregates link stats to get total VHT per link across all hours.
    Inherits from EitherLinkStatsFile for source loading.
    MRO: NetworkVolumesByLink -> EitherLinkStatsFile -> OutputDataFrame -> object
    """
    def __init__(
        self,
        outputDataDirectory: "OutputDataDirectory", # Needed by OutputDataFrame, EitherLinkStatsFile
        source: Union[LinkStatsFromPathTraversals, "LinkStatsFile"], # Needed by EitherLinkStatsFile
        labeledNetwork: LabeledNetwork, # Dependency, likely for geometry/TAZ mapping downstream
        *args,
        **kwargs,
    ):
        # Store labeled network (dependency, not consumed by parents)
        self.labeledNetwork = labeledNetwork

        # Pass common arguments (outputDataDirectory, inputDirectory derived from source)
        # and the source to the EitherLinkStatsFile parent.
        super().__init__(
            outputDataDirectory, # Positional for EitherLinkStatsFile/OutputDataFrame
            source.inputDirectory, # Positional for EitherLinkStatsFile/OutputDataFrame
            source=source, # Keyword-only for EitherLinkStatsFile
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Parameters:
            df (pd.DataFrame): LinkStats DataFrame (indexed by ['link', 'hour']).
        ...
        if df is None or df.empty:
            ...
            return pd.DataFrame(columns=["vht"], index=pd.Index([], name="link"))

        # Ensure required columns and index levels exist
        if 'traveltime' not in df.columns or 'volume' not in df.columns:
            print("Error: Required columns 'traveltime' or 'volume' not found for VHT calculation.")
            return pd.DataFrame(columns=["vht"], index=pd.Index([], name="link"))
        if not isinstance(df.index, pd.MultiIndex) or 'link' not in df.index.names or 'hour' not in df.index.names:
             print("Error: Input DataFrame for NetworkVolumesByLink does not have the expected MultiIndex ['link', 'hour'].")
             return pd.DataFrame(columns=["vht"], index=pd.Index([], name="link"))


        try:
            # Ensure traveltime and volume are numeric
            df['traveltime'] = pd.to_numeric(df['traveltime'], errors='coerce').fillna(0)
            df['volume'] = pd.to_numeric(df['volume'], errors='coerce').fillna(0)


            # Calculate VHT per link-hour
            df["VHT_hour"] = df.traveltime * df.volume / 3600.0 # traveltime likely in seconds

            # Group by link and sum VHT across all hours
            df_agg = df.groupby("link").agg(vht=("VHT_hour", "sum"))

            df_agg.index.name = self.indexedOn # Set index name
            print(f"Finished preprocessing NetworkVolumesByLink ({df_agg.shape[0]} rows).")
            return df_agg
        except Exception as e:
    ...
    # load method is inherited from EitherLinkStatsFile and gets the linkstats data.
    # This data is then passed to this class's preprocess method by the dataFrame property getter.
...
class NetworkVolumesByLinkByIteration(OutputDataFrame):
    ...
    def __init__(
        ...
        labeledNetwork: LabeledNetwork, # Dependency, likely for geometry/TAZ mapping downstream
        iterations: List[int],
        inputType: Optional[str] = "linkStats", # Specifies source ('linkStats' or 'pathTraversals')
        ...
        **kwargs, # Accept args/kwargs
        ...
        super().__init__(outputDataDirectory, inputDirectory, hash_key=override_hash, *args, **kwargs)
    ...
    def load(self) -> pd.DataFrame:
        ...
        """
        print(f"Loading NetworkVolumesByLinkByIteration for iterations {self.iterations} from {self.inputType} source...")
        temp = dict()
        source_class = None
        ...
        if self.inputType.lower() == "linkstats":
            ...
            print(f"Warning: inputType '{self.inputType}' is not fully supported yet. Only 'linkStats' is implemented.")
            source_class = LinkStatsFile # Default to LinkStatsFile even if type is PT

        if source_class:
             for it in sorted(self.iterations): # Process iterations in order for consistent behavior
                ...
                try:
                    ...
                    source_obj = self.inputDirectory.linkStatsFile(it) # Assumes BeamRunInputDirectory input
                    ...
                    nvbl = NetworkVolumesByLink(self.outputDataDirectory, source_obj, self.labeledNetwork)
                    ...
                    if iteration_vht_df is not None and not iteration_vht_df.empty:
                        ...
                        print(f"Successfully loaded VHT for Iteration {it} ({iteration_vht_df.shape[0]} links).")
                    else:
        ...
        if temp:
             ...
             print(f"Finished loading NetworkVolumesByLinkByIteration ({combined_df.shape[0]} rows, {combined_df.shape[1]} columns).")
             return combined_df
        else:
    ...
    def preprocess(self, df):
        ...
        """
        print(f"NetworkVolumesByLinkByIteration preprocess step (no-op). Input shape: {df.shape if df is not None else 'None'}")
        return df
...
class LabeledLinkStatsFile(TAZBasedDataFrame, EitherLinkStatsFile):
    ...
    Inherits from TAZBasedDataFrame (for process/toGdf) and EitherLinkStatsFile (for source loading).
    MRO: LabeledLinkStatsFile -> TAZBasedDataFrame -> EitherLinkStatsFile -> OutputDataFrame -> object
    """
    def __init__(
        self,
        outputDataDirectory: "OutputDataDirectory", # Needed by OutputDataFrame, TAZBasedDataFrame, EitherLinkStatsFile
        source: Union[LinkStatsFromPathTraversals, "LinkStatsFile"], # Needed by EitherLinkStatsFile
        ...
        **kwargs, # Accept any extra kwargs
        ...
        # The inputDirectory comes from the source object
        input_dir_from_source = source.inputDirectory


        # Call super() once, passing ALL arguments needed by parents/grandparents as KEYWORD args
        # Match the parameter names expected by the respective __init__ methods.
        # MRO path: LabeledLinkStatsFile -> TAZBasedDataFrame -> EitherLinkStatsFile -> OutputDataFrame -> object
        super().__init__(
            # Arguments needed by TAZBasedDataFrame.__init__ (outputDataDirectory, inputDirectory, *, geometry, geoIndex, ...)
            outputDataDirectory=outputDataDirectory,
            inputDirectory=input_dir_from_source,
            geometry=geometry,
            geoIndex=geometry.index,

            # Arguments needed by EitherLinkStatsFile.__init__ (outputDataDirectory, inputDirectory, *, source, ...)
            # These also need outputDataDirectory and inputDirectory, already provided above.
            source=source, # Keyword-only argument for EitherLinkStatsFile

            # Arguments needed by OutputDataFrame.__init__ (outputDataDirectory, inputDirectory, *, hash_key=None, ...)
            # These also need outputDataDirectory and inputDirectory, already provided above.
            # Calculate the hash key here based on the source's inputDirectory and the source itself.
            hash_key=LabeledLinkStatsFile._calculate_hash_key(input_dir_from_source, source), # Keyword-only for OutputDataFrame
        ...
        self.indexedOn = ["link", "hour"] # This is correct for the processed state

    @staticmethod
    def _calculate_hash_key(inputDirectory: InputDirectory, source: Union[LinkStatsFromPathTraversals, "LinkStatsFile"]) -> str:
         """Helper method to calculate the hash key for LabeledLinkStatsFile."""
         m = hashlib.md5()
         input_path_str = str(getattr(inputDirectory, 'directoryPath', ''))
         m.update(input_path_str.encode())
         m.update(LabeledLinkStatsFile.__name__.encode()) # Use this class's name
         source_hash_part = f"{source.__class__.__name__}_{getattr(source, 'iteration', 'no_iter')}"
         m.update(source_hash_part.encode())
         return m.hexdigest()
    ...
    def preprocess(self, df):
        """
        Merges LinkStats data with LabeledNetwork attributes.

        Parameters:
            df (pd.DataFrame): LinkStats DataFrame (indexed by ['link', 'hour']).

        Returns:
            pd.DataFrame: LinkStats DataFrame with network attributes and TAZ labels.
        """
        print(f"Preprocessing LabeledLinkStatsFile using mergeLinkstatsWithNetwork ({df.shape[0]} rows)...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            ...
            return pd.DataFrame(
                 columns=['VMT', 'VHT', 'linkLength', 'linkFreeSpeed', 'linkCapacity', 'numberOfLanes', 'linkModes', 'attributeOrigId', 'attributeOrigType', self.geometry.index],
                 index=pd.MultiIndex.from_tuples([], names=['link', 'hour'])
            )

        # Ensure required columns/index exist in df (LinkStats)
        linkstats_required = ['volume', 'traveltime']
        if not all(col in df.columns for col in linkstats_required):
             print(f"Error: Required LinkStats columns {linkstats_required} not found for merge.")
             return pd.DataFrame(
                 columns=['VMT', 'VHT', 'linkLength', 'linkFreeSpeed', 'linkCapacity', 'numberOfLanes', 'linkModes', 'attributeOrigId', 'attributeOrigType', self.geometry.index],
                 index=pd.MultiIndex.from_tuples([], names=['link', 'hour'])
            )
        if not isinstance(df.index, pd.MultiIndex) or 'link' not in df.index.names or 'hour' not in df.index.names:
             print("Error: Input LinkStats DataFrame does not have the expected MultiIndex ['link', 'hour'].")
             return pd.DataFrame(
                 columns=['VMT', 'VHT', 'linkLength', 'linkFreeSpeed', 'linkCapacity', 'numberOfLanes', 'linkModes', 'attributeOrigId', 'attributeOrigType', self.geometry.index],
                 index=pd.MultiIndex.from_tuples([], names=['link', 'hour'])
            )
        ...
        if network_df is None or network_df.empty:
             ...
             return pd.DataFrame(
                 columns=['VMT', 'VHT'], # Return LinkStats with just VMT/VHT if network missing
        ...
        network_required_cols = ['linkLength', 'linkFreeSpeed', 'linkCapacity', 'numberOfLanes', 'linkModes', 'attributeOrigId', 'attributeOrigType', self.geometry.index] # Columns expected from LabeledNetwork

        if network_df.index.name != network_required_index:
             print(f"Error: LabeledNetwork index name is not '{network_required_index}'.")
             return pd.DataFrame(
                 columns=['VMT', 'VHT'],
                 index=df.index
             )
        if not all(col in network_df.columns for col in network_required_cols):
             print(f"Error: Required LabeledNetwork columns {network_required_cols} not found for merge.")
             return pd.DataFrame(
                 columns=['VMT', 'VHT'],
                 index=df.index
             )
        ...
        try:
            ...
            result_df = mergeLinkstatsWithNetwork(
                df, network_df, self.geometry.index
            )
            print(f"Finished preprocessing LabeledLinkStatsFile ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
            print(f"Error during LabeledLinkStatsFile preprocessing: {e}")
            # Return input df with VMT/VHT calculated if merge failed
            if 'volume' in df.columns and 'traveltime' in df.columns:
                 df['VMT'] = df['volume'] * df['linkLength'].fillna(0) / 1609.34 # Need linkLength from network - this will fail if merge failed.
                 df['VHT'] = df['volume'] * df['traveltime'].fillna(0) / 3600.0
                 return df[['VMT', 'VHT']] # Return only calculated VMT/VHT
            return pd.DataFrame(
                 columns=['VMT', 'VHT', 'linkLength', 'linkFreeSpeed', 'linkCapacity', 'numberOfLanes', 'linkModes', 'attributeOrigId', 'attributeOrigType', self.geometry.index],
                 index=pd.MultiIndex.from_tuples([], names=['link', 'hour'])
            )
...
class TAZTrafficVolumes(TAZBasedDataFrame):
    ...
    def __init__(
        ...
        **kwargs, # Accept any extra kwargs
        ...
        self.indexedOn = [self.geometry.index, "hour", "attributeOrigType"] # Set based on aggregation levels
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df is None or df.empty:
            ...
            return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=expected_index_names))

        # Ensure required columns and index levels exist in df (LabeledLinkStatsFile)
        required_cols = ['VMT', 'VHT'] # These should be calculated in LabeledLinkStatsFile.preprocess
        required_index_levels = ['link', 'hour', self.geometry.index, 'attributeOrigType'] # Expected levels after merge

        # Check if required columns exist (VMT, VHT should be created by LabeledLinkStatsFile preprocess)
        if not all(col in df.columns for col in required_cols):
             print(f"Error: Required columns {required_cols} not found in LabeledLinkStatsFile DataFrame.")
             expected_cols = ["VMT", "VHT", "mph"]
             expected_index_names = [self.geometry.index, "hour", "attributeOrigType"]
             return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=expected_index_names))
        ...
        if not all(col in df.columns for col in agg_cols):
             print(f"Error: Required columns {agg_cols} not found in LabeledLinkStatsFile DataFrame for grouping.")
             expected_cols = ["VMT", "VHT", "mph"]
             expected_index_names = [self.geometry.index, "hour", "attributeOrigType"]
             return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=expected_index_names))
        ...
        try:
            ...
            df_agg["mph"] = df_agg["VMT"] / df_agg["VHT"].replace(0, np.nan) # Replace 0 VHT with NaN before division
    ...
    def load(self):
        ...
        if df is not None:
             print(f"Loaded LabeledLinkStatsFile ({df.shape[0]} rows) for TAZTrafficVolumes.")
        else:
...
class MandatoryLocationsByTaz(TAZBasedDataFrame):
    ...
    def __init__(
        ...
        **kwargs, # Accept any extra kwargs
        ...
        super().__init__(
            ...
            geoIndex=geometry.index if geometry else "TAZ", # Keyword-only for TAZBasedDataFrame
            *args, # Pass any extra positional args
            **kwargs, # Pass any extra keyword args
        )
    ...
    def preprocess(self, df):
        ...
        if df is None or df.empty:
            ...
            return pd.DataFrame(
                columns=["population", "jobs"],
                index=pd.Index([], name=self.geoIndex)
            )

        # Ensure required columns exist
        required_cols = [self.geoIndex, "work_zone_id"] # TAZ and work_zone_id needed
        if not all(col in df.columns for col in required_cols):
            print(f"Error: Required columns {required_cols} not found in ProcessedPersonsFile DataFrame for aggregation.")
            return pd.DataFrame(
                columns=["population", "jobs"],
                index=pd.Index([], name=self.geoIndex)
            )
        ...
        try:
            # Count total population by TAZ (using the geoIndex column)
            # Ensure TAZ column is appropriate type if needed, value_counts handles various types
            population = df[self.geoIndex].value_counts().astype(int)

            # Count jobs by work_zone_id (filter out zones <= 0, which are non-work locations)
            # Ensure work_zone_id is numeric before filtering and counting
            df['work_zone_id'] = pd.to_numeric(df['work_zone_id'], errors='coerce')
            workplaces = df["work_zone_id"].value_counts()

            # Filter out non-positive work zone IDs
            workplaces = workplaces.loc[workplaces.index > 0]

            # Rename the index of the workplaces Series to match the TAZ index name
            workplaces.index.set_names(self.geoIndex, inplace=True)

            # Concatenate population and jobs Series into a DataFrame
            # fillna(0) handles TAZs with population but no jobs, or vice versa
            result_df = pd.concat({"population": population, "jobs": workplaces}, axis=1).fillna(0)

            # Ensure the index name is set correctly after concatenation
            result_df.index.name = self.geoIndex

            print(f"Finished preprocessing MandatoryLocationsByTaz ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
    ...
    def load(self):
        ...
        if df is not None:
             print(f"Loaded ProcessedPersonsFile ({df.shape[0]} rows) for MandatoryLocationsByTaz.")
        else:
...
class TripModeCount(TAZBasedDataFrame):
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        print(f"Preprocessing TripModeCount ({df.shape[0]} rows) with indices {self.indices}...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
            ...
            return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=self.indices))
        ...
        if not all(col in df.columns for col in self.indices):
            print(f"Error: Required columns {self.indices} not found in ProcessedTripsFile DataFrame for counting.")
            expected_cols = ["count"]
            return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=self.indices))
        ...
        try:
            ...
            mode_counts = df_mapped.value_counts(self.indices, normalize=False, dropna=False)
...
class TourModeCount(TAZBasedDataFrame):
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"Preprocessing TourModeCount ({df.shape[0]} rows if not None) with indices {self.indices}...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
        ...
        # Ensure required columns (the self.indices) exist in df
        if not all(col in df.columns for col in self.indices):
            print(f"Error: Required columns {self.indices} not found in ProcessedToursFile DataFrame for counting.")
            expected_cols = ["count"]
            return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=self.indices))
        ...
        except (KeyError, AttributeError, Exception) as e: # Catch potential errors like column not found or replace failure
            print(f"Error during TourModeCount preprocessing: {e}. Skipping processing.")
            expected_cols = ["count"]
            return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=self.indices))
...
class TripPMT(OutputDataFrame):
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        print(f"Preprocessing TripPMT ({df.shape[0]} rows) with indices {self.indices}...")
        # Ensure df is not None or empty before processing
        if df is None or df.empty:
        ...
        if not all(col in df.columns for col in required_cols):
            print(f"Error: Required columns {required_cols} not found in DataFrame for PMT aggregation.")
            expected_cols = ["distanceInMiles"]
            return pd.DataFrame(columns=expected_cols, index=pd.MultiIndex.from_tuples([], names=self.indices))
        ...
        try:
            ...
            df_mapped['distanceInMiles'] = pd.to_numeric(df_mapped['distanceInMiles'], errors='coerce').fillna(0)
    ...
    def load(self):
        ...
        if skims is None or skims.empty:
             print("ProcessedSkimsFile is empty or None. Cannot add distances for TripPMT.")
        ...
        if not all(col in trips.columns for col in required_trips_cols):
            print(f"Error: Required trip columns {required_trips_cols} not found for merging skims.")
            return None

        # Ensure required index exists in skims
        required_skims_index = ['Origin', 'Destination']
        if not isinstance(skims.index, pd.MultiIndex) or list(skims.index.names) != required_skims_index:
            print(f"Error: Skims DataFrame does not have the expected index {required_skims_index}.")
            return None
        ...
        try:
            ...
            distance_series.index = trips_valid_loc.index # Align index with the valid trips DF

            trips_valid_loc["distanceInMiles"] = distance_series

            print(f"Loaded trips and added skim distances ({trips_valid_loc.shape[0]} rows).")
...
class MeanDistanceToWork(TAZBasedDataFrame): # Inherit from TAZBasedDataFrame
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not all(col in df.columns for col in required_cols):
            print(f"Error: Required columns {required_cols} not found in DataFrame for aggregation.")
            return pd.DataFrame(
                columns=["meanDistance"],
                index=pd.Index([], name=self.geoIndex)
            )
        ...
        try:
            ...
            # Ensure distanceInMiles is numeric
            df['distanceInMiles'] = pd.to_numeric(df['distanceInMiles'], errors='coerce').fillna(0)
            ...
            byTaz = df.groupby(self.geoIndex).agg(
                {"distanceInMiles": ["sum", "size"]} # Use size for count, it includes NaNs if not dropped
            ...
            byTaz.columns = byTaz.columns.droplevel(0) # Drop the 'distanceInMiles' level from columns

            # Calculate meanDistance, handling division by zero size
            byTaz["meanDistance"] = byTaz["sum"] / byTaz["size"].replace(0, np.nan) # Replace 0 size with NaN
            ...
            print(f"Finished preprocessing MeanDistanceToWork ({result_df.shape[0]} rows).")
            return result_df
        except Exception as e:
    ...
    def load(self):
        ...
        if persons is None or persons.empty:
            print("ProcessedPersonsFile is empty or None. Cannot load for MeanDistanceToWork.")
            return None # Cannot proceed without persons data

        if skims is None or skims.empty:
            print("ProcessedSkimsFile is empty or None. Cannot add distances for MeanDistanceToWork.")
        ...
        persons['work_zone_id'] = pd.to_numeric(persons['work_zone_id'], errors='coerce')
        workers_df = persons.loc[
            persons["work_zone_id"] > 0, [self.geoIndex, "work_zone_id"]
        ].copy() # Select TAZ and work_zone_id, and make a copy

        if workers_df.empty:
             print("No workers found with valid work_zone_id. Returning empty DataFrame.")
             return pd.DataFrame(columns=[self.geoIndex, "work_zone_id", "distanceInMiles"])
        ...
        try:
             ...
             print(f"Loaded workers and added skim distances ({workers_df.shape[0]} rows).")
...
class TripModeCountByOrigin(TripModeCount):
    ...
    def __init__(
        ...
        **kwargs, # Accept extra kwargs
        ...
        geoIndex_for_parent = "origin" # Use origin column as geo index for spatial processing if needed
...
class InfoByYear(OutputDataFrame):
    ...
    def __init__(
        ...
        pilatesInputDict: Dict[
            Tuple[int, int], "ModelOutputData"
        ],
        accessor: Callable[
            ["ModelOutputData"], Optional[pd.DataFrame]
        ],
        ...
        **kwargs,
        ...
        if pilatesInputDict:
            for yr, it in pilatesInputDict.keys():
                if yr not in last_iteration_per_year or it >= last_iteration_per_year[yr]:
        ...
        m.update(input_path_str.encode())
        m.update(self.__class__.__name__.encode())
        m.update(str(sorted(self.__lastIterationPerYear.items())).encode())  # Include years and their LAST iteration
        override_hash = m.hexdigest()
    ...
    def load(self):
        ...
        if self.__lastIterationPerYear:
            for yr in sorted(self.__lastIterationPerYear.keys()):
                ...
                iterations_for_year = sorted([i for (y, i) in self.pilatesInputDict.keys() if y == yr], reverse=True)

                # Find the index of the target 'it' in the sorted iterations for the year
                try:
                    start_idx = iterations_for_year.index(it)
                except ValueError:
                    print(f"Warning: Last iteration {it} for year {yr} not found in pilatesInputDict keys. Skipping year.")
                     ...
                     try:
                         if (yr, current_it) in self.pilatesInputDict:
                             ...
                             if df is not None and not df.empty:
                              ...
                              print(f" - Run data for year {yr}, iteration {current_it} not found unexpectedly. Skipping.") # Should not happen within sorted iterations_for_year
            ...
            if yr not in data_frames: # Check if data was successfully loaded for the year after trying iterations
                 print(
        ...
        if not data_frames:
            ...
            return pd.DataFrame(columns=[]).set_index(pd.MultiIndex.from_tuples([], names=concat_names_if_empty))
        ...
        first_df_index_names = list(first_df_index.names or []) # Use list() to handle None
        ...
        if not valid_index_names and isinstance(
            ...
            data_frames = {k: v.to_frame(name=series_name) for k, v in data_frames.items()} # Convert Series to DF with a column name
             ...
             if first_df.index.name is None and isinstance(first_df.index, pd.RangeIndex):
                 print(f"Warning: Accessor returned DataFrame with default integer index for {self.__class__.__name__}. Concatenated columns will include original DF columns.")
        ...
        self.indexedOn = list(combined_df.index.names) if combined_df.index.names else []
...
class TripPMTByPrimaryPurposeByYear(InfoByYear):
    ...
    def __init__(
        ...
        columns = ["trip_mode", "primary_purpose"] # Expected index names of the accessor's output
...
class InfoByIteration(OutputDataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        m.update(input_path_str.encode())
        m.update(self.__class__.__name__.encode())
        m.update(str(sorted(pilatesInputDict.keys())).encode())  # Include all years/iters
        override_hash = m.hexdigest()
            ...
            for yr, it in sorted(self.pilatesInputDict.keys()):
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        ...
                        print(f" - Loaded year {yr}, iteration {it} ({df.shape[0]} rows)")
...
class TripPMTByCountyByYear(TAZBasedDataFrame, InfoByYear):
    ...
    def __init__(
        ...
        pilatesInputDict: Dict[Tuple[int, int], "ActivitySimRunOutputData"], # Needed by InfoByYear
    ):
        # Define the accessor function to get the data for a single year/iteration
        ...
        def accessor(outputData: "ActivitySimRunOutputData") -> Optional[pd.DataFrame]:
            ...
            if pmt_by_origin_df is None or pmt_by_origin_df.empty:
                print(f"Warning: TripPMTByOrigin data is empty or None for run {outputData.inputDirectory.directoryPath}.")
            ...
            if 'county' not in df_merged.columns:
                 print(f"Error: 'county' column not added after merging with geometry for run {outputData.inputDirectory.directoryPath}. Cannot aggregate by county.")
                 # Return empty DF with expected index levels ['county', 'trip_mode'] and column 'distanceInMiles'
                 return pd.DataFrame(
                    columns=["distanceInMiles"],
                    index=pd.MultiIndex.from_tuples([], names=["county", "trip_mode"]),
            ...
            df_merged['distanceInMiles'] = pd.to_numeric(df_merged['distanceInMiles'], errors='coerce').fillna(0)
            ...
            print(f"Finished accessor aggregation for run {outputData.inputDirectory.directoryPath} ({aggregated_df.shape[0]} rows).")
            return aggregated_df


        # The index names of the accessor's output DF are ['county', 'trip_mode'].
        columns_for_info_by = ["county", "trip_mode"] # Expected index names from accessor output
...
class MandatoryLocationByTazByYear(TAZBasedDataFrame, InfoByYear):
    ...
    def __init__(
        ...
        columns_for_info_by = [geometry.index] # Expected index names from accessor output
...
class TripModeCountByIteration(InfoByIteration):
    ...
    def __init__(
        ...
        columns_for_info_by = ["trip_mode"]  # Expected index name of the accessor's output DF
...
class TourModeCountByIteration(InfoByIteration):
    ...
    def __init__(
        ...
        columns_for_info_by = ["tour_mode"]  # Expected index name of the accessor's output DF
        ...
        # The final index after load (from InfoByIteration) will be [year, iteration, tour_mode].
        # The indexedOn attribute will be set by InfoByIteration's load() method.
        # self.indexedOn = ["year", "iteration", "tour_mode"] # Set by InfoByIteration.load()
...
class ReplanningEventReasonByIteration(InfoByIteration):
    ...
    def __init__(
        ...
        columns_for_info_by = ["reason"]  # Expected index name of the aggregated dataframe
        ...
        # The final index after load (from InfoByIteration) will be [year, iteration, reason].
        # The indexedOn attribute will be set by InfoByIteration's load() method.
        # self.indexedOn = ["year", "iteration", "reason"] # Set by InfoByIteration.load()
...
class ScoreStatsByIteration(InfoByIteration):
    ...
    def __init__(
        ...
        # If it has a default index, InfoByIteration will create a MultiIndex
        # [year, iteration, original_index].
        # If it has a meaningful index and columns, those are preserved.
        # Since the structure isn't fixed/aggregated here, the `columns` list
        # for InfoByIteration should represent the expected *original* index levels.
        # Assuming scorestats.txt has a simple structure, maybe no specific index levels are expected?
        # Or maybe the index is 'person_id'? Let's check the raw file structure.
        # Looking at src/input.py, ScoreStatsFile uses pd.read_table with index_col=self.index_col (which is None by default).
        # So the raw DataFrame has a default integer index and columns from the file.
        # The InfoByIteration load method will concatenate these, resulting in an index [year, iteration, default_integer_index].
        # The `columns` list should describe the index *of the accessor's output*. If the accessor's output
        # has a default index with no name, `InfoByIteration.load` handles this by adding year/iteration and keeping original columns.
        # Setting columns=[] seems appropriate here to indicate no specific index levels are expected from the accessor's raw DF.
        columns_for_info_by = [] # No specific index levels expected from accessor's output DataFrame
...
class TripModeCountByCountyByYear(TAZBasedDataFrame, InfoByYear):
    ...
    def __init__(
        ...
        def accessor(outputData: "ActivitySimRunOutputData") -> Optional[pd.DataFrame]:
            ...
            if df is None or df.empty:
                print(f"Warning: TripModeCountByOrigin data is empty or None for run {outputData.inputDirectory.directoryPath}.")
            ...
            try:
                aggregated_df = outputData.tripModeCountByOrigin.process(
                    ...
                    aggregateBy=["county", "trip_mode"], # Group by county (from geo merge) and trip_mode (index level)
                ...
                if not isinstance(aggregated_df.index, pd.MultiIndex) or list(aggregated_df.index.names) != ["county", "trip_mode"]:
                     print(f"Warning: Accessor aggregation did not result in expected index ['county', 'trip_mode'] for run {outputData.inputDirectory.directoryPath}. Actual index names: {list(aggregated_df.index.names)}")
                     # Attempt to reset/set index
                     if all(col in aggregated_df.columns for col in ["county", "trip_mode"]):
                         aggregated_df = aggregated_df.reset_index().set_index(["county", "trip_mode"])
                         aggregated_df.index.set_names(["county", "trip_mode"], inplace=True)
                     else:
                          print("Error: Cannot set index ['county', 'trip_mode'] as required columns are missing after aggregation.")
                          return pd.DataFrame(
                             columns=["count"],
                             index=pd.MultiIndex.from_tuples([], names=["county", "trip_mode"]),
                         )
                ...
                print(f"Error during accessor aggregation for run {outputData.inputDirectory.directoryPath}: {e}")
...
class ModeVMTByYear(InfoByYear):
    ...
    def __init__(
        ...
        outputDataDirectory: "OutputDataDirectory", # Needed by OutputDataFrame, InfoByYear
        pilatesRunInputDirectory: PilatesRunInputDirectory, # Needed by OutputDataFrame, InfoByYear
        pilatesInputDict: Dict[Tuple[int, int], "BeamRunOutputData"], # Needed by InfoByYear
    ):
        def accessor(outputData: "BeamRunOutputData") -> Optional[pd.DataFrame]:
        ...
        columns_for_info_by = ["mode_extended"]  # Expected index name of the accessor's output DF
...
class TripsByYear(InfoByYear):
    ...
    def __init__(
        ...
        def accessor(outputData: "BeamRunOutputData") -> Optional[pd.DataFrame]:
             ...
             asim_input_dir = pilatesRunInputDirectory.asimRuns.get((outputData.inputDirectory.year, outputData.inputDirectory.iteration))
             if asim_input_dir is None:
                 print(f"Warning: Corresponding ASIM input directory not found for Beam run {outputData.inputDirectory.directoryPath}. Cannot aggregate trips.")
                 return pd.DataFrame() # Return empty DF

             # Call the getAggregatedTrips method on the BeamOutputData instance
             # Pass the corresponding ASIM input directory
             print(f"Calling getAggregatedTrips for {outputData.inputDirectory.directoryPath}...")
             aggregated_trips_df = outputData.getAggregatedTrips(asim_input_dir)
             print(f"getAggregatedTrips returned shape {aggregated_trips_df.shape if aggregated_trips_df is not None else 'None'}.")
...
class CongestionInfoByYear(TAZBasedDataFrame, InfoByYear):
    ...
    def __init__(
        ...
        # Call __InfoByYear.__init__ first
